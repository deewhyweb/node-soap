...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    // *****************************************************************
    // Saving Requests to FeedHenry DB to ensure audit trail
    // console.log( 'soap request:: ' + JSON.stringify( xml ) );
    //
    console.log( 'about to call out from client ****************************************************************************************************************************************************************************************************************************************************************************************************' );
    ...
    http.request(location, xml, function(err, response, body) {
        saveClientRequestResponse(xml, err, response, body);
        if (err) {
            callback(err);
        }
            ...
            callback(new Error('Non 200 Status:' + response.statusCode));
        } else {
            callback(new Error('Body is empty'));
        }
    }, headers, options);
}
...
function saveClientRequestResponse (xml, err, response, body) {
    // *****************************************************************
    // Saving Requests to FeedHenry DB to ensure audit trail
    console.log( 'about to call out from http ****************************************************************************************************************************************************************************************************************************************************************************************************' );
    var datetime = new Date();
    $fh.db({
        act: "create",
        type: "woSoapXMLOut",
