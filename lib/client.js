...
Client.prototype._invoke = function(method, arguments, location, callback) {
    ...
    console.log('About to send http soap request');

    http.request(location, xml, function(err, response, body) {

        if (err) {
            console.error('error occurred in soap request.'+err);
            callback(err);
        }
        else if (response.statusCode === 200 && body != null) {
            console.log( 'Got 200 http soap response from FH client request.' );
            try {
            ...
            console.error( 'Non 200 Status from http soap request.'+response.statusCode );
            callback(new Error('Non 200 Status:' + response.statusCode));
        } else {
            console.error( 'Body is empty.'+response.statusCode );
