...
var Element = function(nsName, attrs) {
    ...
    for(var defaultKey in DEFAULT_ATTRIBUTES) {
        if(this['$' + defaultKey] === undefined) {
            this['$' + defaultKey] = DEFAULT_ATTRIBUTES[defaultKey];
        }
    }
...
ElementElement.prototype.description = function(definitions) {
    ...
    if((maxOccurs > 1 || maxOccurs === 'unbounded') && minOccurs !== maxOccurs) {
        name += '[]';
    }
...
WSDL.prototype.xmlToObject = function(xml) {
    ...
    p.on('text', function(text) {
        ...
        // bg hack => adding
        if( !top.schema || typeof top.schema === "string" ) {
            name = splitNSName(top.schema).name;

            // don't trim text if should be a string
            if(name !== "string") {
                text = trim(text);
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            if (i > 0) {
                ...
                // a name can be made up of elements and attributes - we may need to split them
                // The ' ' character separates an element and attribute
                var eleDtls = name.split(' ');
                name = eleDtls[0];
                attrs = eleDtls[1] ? ' '+eleDtls[1] : '';

                parts.push(['</',ns,name,'>'].join(''));
                parts.push(['<',ns,name,xmlnsAttrib,attrs,'>'].join(''));
            }
