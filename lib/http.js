...
exports.request = function(rurl, data, callback, exheaders, exoptions) {
    ...
    console.log('About to save http soap request');
    ...
    saveHttpRequestResponse(data, function(err, dbRes) {
        ...
        if (err) {
            ...
            console.log('Successfully saved soap request');
...
function getWoNum (data) {
    ...
    try {
        woNumRegex = /\<ns\:WONUM\>(.+?)\<\/ns\:WONUM\>/g;
...
function getAssetNum (data) {
    var assetNumRegex,
        assetNum,
        assetNumId;

    try {
        assetNumRegex = /\<ns\:ASSETNUM\>(.+?)\<\/ns\:ASSETNUM\>/g;
        assetNum = assetNumRegex.exec( data );
        assetNumId = assetNum[1];
    } catch (e) {
    }
