        ...
        req.on('end', function() {
            ...
            try {
                self._process(xml, req.url, function(result) {
                    saveServerRequestResponse(xml, null, result);
                    res.write(result);
                    res.end();
                ...
                saveServerRequestResponse(xml, err, null);
                err = err.stack || err;
                res.write(err);
                res.end();
                if (typeof self.log === 'function') {
...
Server.prototype._process = function(input, URL, callback) {
    ...
    if (typeof self.authenticate === 'function') {
      ...
      if (obj.Header == null || obj.Header.Credentials == null) {
        throw new Error('No security header');
      }
      if (!self.authenticate(obj.Header.Credentials)) {
        throw new Error('Invalid username or password');
      }
    }

    // Original Node-SOAP code
...
function saveServerRequestResponse (xml, err, body) {

    var datetime, parentWoNum, woNum;

    var data = new String(xml);

    // remove whitespace and carriage returns from xml
    data = data.replace(/ /g,'');
    data = data.replace(/[\n\r]/g, '');
    data = data.replace(/\r\n/g, '');
    ...
    parentWoNum = getParentWoNum(data);
    woNum = getWoNum(data);
    ...
    $fh.db({
        ...
        if (err) {
            // console.log( 'Problem saving soap xml to DB' + err );
        } else {
            // console.log( 'Saved soap xml data to db: ' + JSON.stringify( res ) );
        }
    });
}
function getWoNum (data) {
    var woNumRegex,
        ...
        woNumId = '';
    try {
        woNumRegex = /WONUM\<\/gbdt\:name>\<gbdt\:type>TEXT\<\/gbdt\:type>\<gbdt\:value>\<gbc\:stringValue>(.+?)\<\/gbc\:stringValue>/g;
...
function getParentWoNum (data) {
    var woNumRegex,
        woNum,
        woNumId = '';
    try {
        woNumRegex = /PARENT\<\/gbdt\:name>\<gbdt\:type>TEXT\<\/gbdt\:type>\<gbdt\:value>\<gbc\:stringValue>(.+?)\<\/gbc\:stringValue>/g;
        woNum = woNumRegex.exec( data );
        woNumId = woNum[1];
    } catch (e) {
    }
